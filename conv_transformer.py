...
class PE(nn.Module):
    ...
    def __init__(self, d_model, max_len=5000):
        ...
        self.learnable_pe = nn.Parameter(torch.randn(max_len, 1, d_model) * 0.02)

    def forward(self, x):
        return x + self.pe[:x.size(0), :] + self.learnable_pe[:x.size(0), :, :]
...
class TransformerTimeSeriesModel(nn.Module):
    def __init__(self, input_dim, output_dim, seq_length, output_length, 
        ...
        self.conv_encoder_pre = nn.Conv1d(
            in_channels=d_model,
        ...
        self.conv_encoder_post = nn.Conv1d(
            in_channels=d_model,
            out_channels=d_model,
            kernel_size=3,
            padding=2,
        ...
        self.rescale = nn.Linear(d_model, 1)
    ...
    def forward(self, src):
        ...
        # 在编码器处理时增加残差权重
        src_conv = src.permute(1, 2, 0)  # [batch, d_model, seq_len]
        src_conv = self.conv_encoder_pre(src_conv)
        src_conv = src_conv.permute(2, 0, 1)  # [seq_len, batch, d_model]
        memory = 0
        for i, layer in enumerate(self.transformer_encoder.layers):
            src = layer(src_conv, self.encoder_mask.to(src_conv.device))
            memory += self.residual_weights[i] * src
        memory = memory / len(self.transformer_encoder.layers)

        # 编码器后的卷积处理及残差连接
        memory_conv = memory.permute(1, 2, 0)  # [batch, d_model, seq_len]
        memory_conv = self.conv_encoder_post(memory_conv)
        memory_conv = memory_conv.permute(2, 0, 1)  # [seq_len, batch, d_model]
        memory = memory + memory_conv
...
postprocess(np_pred_array,np_true_array,model_name="transformer_res",seq_length=seq_length, output_length=output_length)
